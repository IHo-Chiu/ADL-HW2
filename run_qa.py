...
class ModelArguments:
    ...
    load_pretrained_weight: bool = field(
        default=True,
        metadata={
            "help": (
                "Load pretrained weight or not."
            )
        },
    )
...
class DataTrainingArguments:
    ...
    output_file: Optional[str] = field(default=None, metadata={"help": "Where to store the final prediction."})
...
def main():
    ...
    if not model_args.load_pretrained_weight:
    ...
    def prepare_train_features(examples):
        ...
        contexts = [context_json[idx] for idx in examples[context_column_name]]
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        if training_args.do_predict:
            references = [{"id": ex["id"], "answers": {"text": "","start": 0}} for ex in examples]
        else:
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for i in range(len(predictions)):
            ...
            label = answers[i]['answers']['text']
            metrics.append(int(''.join(label) == ''.join(prediction)))

        return {'accuracy': sum(metrics) / len(metrics)}
    ...
    if training_args.do_predict:
        ...
        with open(data_args.output_file, 'w', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(['id', 'answer'])
            for i, pred in enumerate(results.predictions):
                csv_writer.writerow([pred['id'], pred['prediction_text']])
